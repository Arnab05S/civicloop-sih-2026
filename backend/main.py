...
# Lightweight, explainable semantic matching for the Render free tier. It normalizes common
# civic-issue synonyms and compares weighted concepts plus the original text shape. This avoids
# downloading or keeping an ML model in memory while still catching paraphrased nearby reports.
STOP_WORDS = {"a", "an", "and", "at", "by", "for", "from", "in", "is", "it", "near", "of", "on", "the", "to", "with"}
PHRASE_CONCEPTS = {
        "water logging": "waterlog", "waterlogged": "waterlog", "water log": "waterlog",
        "street light": "streetlight", "lamp post": "streetlight", "lamp pole": "streetlight",
        "not working": "broken", "stopped working": "broken", "out of order": "broken",
        "pothole": "roaddefect", "crater": "roaddefect", "road hole": "roaddefect", "deep break": "roaddefect",
        "overflowing": "overflow", "overflow": "overflow", "spilling": "overflow",
        "clogged": "blocked", "blockage": "blocked", "debris": "blocked",
        "garbage": "waste", "rubbish": "waste", "trash": "waste", "bins": "waste",
        "footpath": "sidewalk", "pavement": "sidewalk", "walkway": "sidewalk",
}
TOKEN_CONCEPTS = {"flooding": "waterlog", "flooded": "waterlog", "drainage": "drain", "drains": "drain", "drain": "drain", "lights": "streetlight", "light": "streetlight", "road": "road", "roads": "road", "damaged": "broken", "damage": "broken"}

def semantic_terms(text: str) -> Counter:
        normalized = text.lower()
        for phrase, concept in PHRASE_CONCEPTS.items():
                    normalized = normalized.replace(phrase, f" {concept} ")
                tokens = re.findall(r"[a-z0-9]+", normalized)
    concepts = []
    for token in tokens:
                if token in STOP_WORDS or len(token) < 3:
                                continue
                            token = TOKEN_CONCEPTS.get(token, token)
...
def lightweight_similarity(title_a: str, detail_a: str, title_b: str, detail_b: str) -> float:
        terms_a = semantic_terms(f"{title_a} {title_a} {detail_a}")
    terms_b = semantic_terms(f"{title_b} {title_b} {detail_b}")
    if not terms_a or not terms_b:
                return 0.0
    shared_weight = sum((terms_a & terms_b).values())
    # Coverage prevents a longer citizen description from diluting the shared issue concepts.
    concept_coverage = shared_weight / min(sum(terms_a.values()), sum(terms_b.values()))
    shape_score = SequenceMatcher(None, " ".join(sorted(terms_a)), " ".join(sorted(terms_b))).ratio()
    return round(0.65 * concept_coverage + 0.35 * shape_score, 3)
        ...
        if nearby:
                        for r in nearby:
                                            if distance_m(payload.lat, payload.lng, r["lat"], r["lng"]) > 250:
                                                                    continue
                                                                similarity = lightweight_similarity(payload.title, payload.detail, r["title"], r["detail"])
                                            if similarity >= 0.50:
                                                                    match = r
                                                                    break
